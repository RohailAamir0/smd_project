...
import type { TabParamList, AppStackParamList, Transaction, Category } from '../types';
...
const ALL_FILTER = { id: 'all', label: 'All', icon: 'format-list-bulleted', color: Colors.accent1 };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('AddTransaction', undefined)}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={[styles.chip, activeFilter === item.id && { backgroundColor: item.color + '33', borderColor: item.color }]}
          ...
          >
            ...
            />
            <Text style={[styles.chipText, activeFilter === item.id && { color: item.color }]}>
              {item.label}
            </Text>
          </TouchableOpacity>
        )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <MaterialCommunityIcons name="receipt-outline" size={52} color={Colors.textDim} />
...
const styles = StyleSheet.create({
  ...
  typeBtnText:       { color: Colors.textMuted, fontSize: FontSize.sm, fontWeight: FontWeight.semibold },
